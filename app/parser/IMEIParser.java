...
/**
 * Created by penpen on 30.03.17.
 */
public class IMEIParser extends Parser <IMEILesson> {

   @Deprecated
   public List<IMEILesson> parseFile(File file) throws IOException {
       return parseStream(new FileInputStream(file));
   }


   public List<IMEILesson> parseURL(URL url) throws IOException {
       return parseStream(url.openStream());
   }


   public List<IMEILesson> parseStream(InputStream in) throws IOException {
       List<IMEILesson> list = new ArrayList<IMEILesson>();
   ...
   public List<IMEILesson> parseSheet(HSSFSheet sheet) {
       List<IMEILesson> list = new ArrayList<IMEILesson>();
       ...
       for (int y = 2; !endOfColumns(startLine, y, dataBase); y += 3) {
           ...
           groupName = getGroupName(startLine,y,dataBase);
               ...
               if (notEmpty(lecture)) {
                   ...
                   if (lecture.contains("\n") && room.contains("\n") && instructor.contains("")) {
                       ...
                       for (int i = 0; i < lecs.length; i++) {
                           IMEILesson lesson = new IMEILesson();
       ...
       for (int i = 0; i < list.size() - 1; i++) {
           IMEILesson lesson1 = list.get(i);
           IMEILesson lesson2 = list.get(i + 1);
   ...
   private static boolean isUpperLowerWeekPair(IMEILesson lesson1, IMEILesson lesson2) {
